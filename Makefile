...
deploy-zk:
	forge create src/MerkleAirdrop.sol:MerkleAirdrop --rpc-url http://127.0.0.1:8011 --private-key ${DEFAULT_ZKSYNC_LOCAL_KEY} --constructor-args ${ROOT} $(shell forge create src/VacuumToken.sol:VacuumToken --rpc-url http://127.0.0.1:8011 --private-key ${DEFAULT_ZKSYNC_LOCAL_KEY} --legacy --zksync | grep "Deployed to:" | awk '{print $$3}') --legacy --zksync

deploy-zk-sepolia:
	forge create src/MerkleAirdrop.sol:MerkleAirdrop --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --constructor-args ${ROOT} $(shell forge create src/VacuumToken.sol:VacuumToken --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account sepoliaTestKey --legacy --zksync | grep "Deployed to:" | awk '{print $$3}') --account sepoliaTestKey --legacy --zksync

# Generate Merkle Tree input file
generate :; forge script script/GenerateInput.s.sol:GenerateInput

# Make Merkle Root and Proofs
create :; forge script script/CreateMerkle.s.sol:CreateMerkle

# Generate input and output files 

merkle :; forge script script/GenerateInput.s.sol:GenerateInput && forge script script/CreateMerkle.s.sol:CreateMerkle

zktest :; foundryup-zksync && forge test --zksync && foundryup

sign :; 
	@cast wallet sign --no-hash --private-key $(DEFAULT_ANVIL_KEY_2) ${shell cast call ${AIRDROP_ADDRESS} "getMessageHash(address,uint256)" ${DEFAULT_ANVIL_ADDRESS_2} ${AIRDROP_AMOUNT} --rpc-url http://localhost:8545}

claim:;
	@forge script script/Interaction.s.sol:ClaimAirdrop --sender ${DEFAULT_ANVIL_ADDRESS} --rpc-url http://localhost:8545 --private-key $(DEFAULT_ANVIL_KEY) --broadcast

balance :; 
	@cast --to-dec ${shell cast call ${TOKEN_ADDRESS} "balanceOf(address)" ${DEFAULT_ANVIL_ADDRESS_2} --rpc-url http://localhost:8545}
